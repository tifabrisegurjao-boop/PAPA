...
- `dados/base.json` é servido só pelo `npm run dev` e embutido só no `build:demo` (plugin em `vite.config.ts`); o build de produção não leva base nenhuma. Está **commitado de propósito** (decisão de 14/09/2026): é a base fictícia grande (73 clientes / 259 processos, da planilha migrada) que a demo pública do GitHub Pages usa para testes em escala. Para voltar à base pequena: `npm run dados:exemplo`. Os links da pasta do OneDrive dessa base são de exemplo (`npm run dados:links-exemplo` preenche quem não tem link com `https://onedrive.live.com/`, só para o cartão ficar clicável nos testes — nunca rodar numa base real). Depois, `npm run dados:enriquecer-demo` deixa as pastas cheias para apresentação: completa objeto, status, situação atual, última movimentação, link e prazo de acesso de todos os processos e acrescenta desdobramentos (recursos, comunicações, manifestações, certidões) e alguns processos principais em outros órgãos — 259 → 438 processos, determinístico e sem duplicar se rodar de novo. Tudo inventado; só para a base fictícia. Sequência completa da demo: `npm run dados -- planilha_migrada.xlsx` → `npm run dados:links-exemplo` → `npm run dados:enriquecer-demo`.
